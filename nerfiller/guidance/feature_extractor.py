...
from nerfiller.guidance.superpoint import SuperPoint
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, image: Float[Tensor, "B 3 H W"] = None):
        pass


class LoFTRExtractor(FeatureExtractor):
    """
    LoFTR feature extractor.
    """

    def __init__(self, device: str = "cuda"):
        super().__init__(device=device)
        self.loftr = LoFTR("outdoor").to(self.device)

    def forward(self, image: Float[Tensor, "B 3 H W"] = None):
        # TODO(ethan): maybe tack on color so we don't lose that valuable texture signal
        gray = rgb_to_grayscale(image)
        feats_c, feats_f = self.loftr.backbone(gray)
        feats_c = torch.nn.functional.interpolate(feats_c, size=feats_f.shape[-2:])
        feats = torch.cat([feats_c, feats_f], dim=1)
        return feats
...
class SuperPointExtractor(FeatureExtractor):
    ...
    def __init__(self, device: str = "cuda"):
        ...
        self.superpoint = SuperPoint(config).to(self.device)

    def forward(self, image: Float[Tensor, "B 3 H W"] = None):
        gray = rgb_to_grayscale(image)
        pred = self.superpoint({"image": gray})
        features = pred["features"]
        # tack on colors since superpoint operates on grayscale images
        colors = torch.nn.functional.interpolate(image, size=features.shape[-2:])
        # r = max(features.shape[1] // 3, 1)  # number of times to repeat the color dim
        r = 1
        features = torch.cat([features, colors.repeat(1, r, 1, 1)], dim=1)
        return features
...
class ViTExtractor(FeatureExtractor):
    ...
    def preprocess(
        self, image: torch.Tensor, load_size: Union[int, Tuple[int, int]] = None
    ...
    def forward(self, image: Float[Tensor, "B 3 H W"] = None):
        ...
        dino_bin = False
        dino_load_size = 500
        imagepre = self.preprocess(image, dino_load_size)[0]
        features = self.extract_descriptors(
            imagepre,
            [dino_layer],
            dino_facet,
            dino_bin,
        )
        features = features.reshape(image.shape[0], self.num_patches[0], self.num_patches[1], -1).permute(0, 3, 1, 2)
        return features
